...
// the theme foreground instead of the brand purple gradient — the same
...
// two overlapping rounded squares. We keep them as two separate layers:
// the back square at a lower alpha, the front square opaque, so the mark
// reads with the same front/back depth as the brand logo while staying a
// single theme colour. State (unlocked / signed out) is carried by the
// caller's `color` and `opacity`, like the sibling plugins.
//
...
Item {
  ...
  // Alpha of the rear square. Lower = more contrast between the two lobes.
  ...
  Item {
    ...
    // curves stay crisp instead of being rasterised and stretched.
